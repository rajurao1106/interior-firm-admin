...
export default function ClientDetails() {
  ...
  const fetchProposals = async (clientData: any) => {
    ...
    try {
      const clientProjectIds: number[] = (clientData?.projects || []).map((p: any) => p.id);
      const r = await fetch(`${API}/proposals/?client=${id}`);
  ...
  const fetchQuotations = async (clientData: any) => {
    ...
    try {
      const clientProjectIds: number[] = (clientData?.projects || []).map((p: any) => p.id);
      const r = await fetch(`${API}/quotations/?client=${id}`);
      const d = await r.json();
  ...
  const fetchClientDataAndReturn = async () => {
    try {
      const r = await fetch(`${API}/clients/${id}/`);
  ...
  useEffect(() => { fetchClientDataAndReturn(); }, [id]);
